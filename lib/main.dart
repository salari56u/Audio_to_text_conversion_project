...
Future<void> requestMicrophonePermission() async {
  ...
  if (status.isGranted) {
    ...
    // Proceed with accessing the microphone
  } else if (status.isPermanentlyDenied) {
    print("Microphone permission is permanently denied. Open settings to enable it.");
    openAppSettings();
  } else {
  ...
  Future<void> initSpeechState() async {
    ...
    try {
      ...
      if (hasSpeech) {
        ...
        // Set system locale and add Persian (fa-IR) if available
        var systemLocale = await speech.systemLocale();
        _currentLocaleId = systemLocale?.localeId ?? 'fa_IR'; // Default to Persian
      }
      if (!mounted) return;
...
// Other widget classes remain unchanged (RecognitionResultsWidget, HeaderWidget, etc.)
